...
def check_parameter_status(model):
    """Check if vision components are properly frozen."""
    vision_frozen = mm_frozen = 0
    vision_total = mm_total = 0
    ...
    if vision_total > 0 and vision_frozen == vision_total and mm_total > 0 and mm_frozen == mm_total:
        rank0_print("Vision components successfully frozen")
    elif vision_total == 0 and mm_total == 0:
        rank0_print("No vision components found")
    else:
        rank0_print(f"Warning: Vision {vision_frozen}/{vision_total}, MM Projector {mm_frozen}/{mm_total} frozen")
